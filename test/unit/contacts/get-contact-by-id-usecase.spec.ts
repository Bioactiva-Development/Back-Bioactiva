...
describe('Contacts module', () => {
	describe('GetContactByIdUseCase', () => {
		...
		it('should return contact when found', async () => {
			const contact = { id: 1, nombres: 'Juan', organizationName: 'Org A' };
			mockRepository.findByIdWithOrganization.mockResolvedValue(contact);

			const result = await useCase.execute(1);

			expect(result).toEqual(contact);
			expect(mockRepository.findByIdWithOrganization).toHaveBeenCalledWith(1);
		});
		...
		it('should throw ContactNotFoundException when not found', async () => {
			...
			await expect(useCase.execute(999)).rejects.toThrow(ContactNotFoundException);
		});

		it('should propagate repository errors', async () => {
			mockRepository.findByIdWithOrganization.mockRejectedValue(new Error('DB error'));
