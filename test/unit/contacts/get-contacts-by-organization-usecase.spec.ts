...
describe('Contacts module', () => {
	describe('GetContactsByOrganizationUseCase', () => {
		...
		it('should return contacts for organization', async () => {
			...
			mockRepository.findByOrganizationIdWithOrganization.mockResolvedValue(contacts);

			const result = await useCase.execute('org-1');

			expect(result).toEqual(contacts);
			expect(mockRepository.findByOrganizationIdWithOrganization).toHaveBeenCalledWith('org-1');
		});
		...
		it('should propagate repository errors', async () => {
			mockRepository.findByOrganizationIdWithOrganization.mockRejectedValue(new Error('DB error'));
