...
describe('Security module', () => {
	...
	describe('Authentication and session management', () => {
		...
		it('should authenticate a valid user and issue a token pair', async () => {
			...
			await expect(
				useCase.execute(new LoginCredentials('ana@bioactiva.com', 'secret')),
			).resolves.toEqual(
		...
		it('should reject login when the password does not match', async () => {
			...
			await expect(
				useCase.execute(new LoginCredentials('ana@bioactiva.com', 'wrong')),
			).rejects.toThrow('Credenciales inválidas');
			expect(tokenService.signAccessToken).not.toHaveBeenCalled();
		});
		...
		it('should allow the refresh use case to issue new tokens for a valid session', async () => {
			const activeUser = buildActiveUser();
			const tokenPair = new TokenPair('new-access', 'new-refresh', 900, 604800);
			...
			await expect(useCase.execute('refresh-token')).resolves.toEqual(tokenPair);
