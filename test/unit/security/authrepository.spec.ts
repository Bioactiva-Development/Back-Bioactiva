	...
	describe('Prisma user auth repository', () => {
		...
		describe('findByCorreo', () => {
			it('should return user when email exists', async () => {
				prismaMock.usuario.findUnique.mockResolvedValue(mockUserRecord as never);
				jest.spyOn(UserMapper, 'toDomain').mockReturnValue(buildUser());

				const result = await repository.findByCorreo('ana@bioactiva.com');
			...
			it('should return null when email does not exist', async () => {
				...
				const result = await repository.findByCorreo('notfound@bioactiva.com');
		...
		describe('save', () => {
			...
			prismaMock.usuario.update.mockResolvedValue(
				{ ...mockUserRecord, estado: 'ACTIVO' } as never,
			);
			jest.spyOn(UserMapper, 'toDomain').mockReturnValue(inactiveUser);
			...
			it('should map user role to ADMINISTRADOR', async () => {
				...
				prismaMock.usuario.update.mockResolvedValue(
					{ ...mockUserRecord, id: 2, rol: 'ADMINISTRADOR' } as never,
				);
