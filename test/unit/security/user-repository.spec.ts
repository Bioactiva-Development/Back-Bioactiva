	...
	describe('PrismaUserRepository', () => {
		...
		describe('findAll', () => {
			it('should return all users with default pagination', async () => {
				(mockPrismaClient.usuario.findMany as jest.Mock).mockResolvedValue([mockUserData]);
			...
			it('should apply search filter correctly', async () => {
				(mockPrismaClient.usuario.findMany as jest.Mock).mockResolvedValue([]);

				await repository.findAll({ search: 'juan', page: 1, limit: 5 });

				const callArgs = (mockPrismaClient.usuario.findMany as jest.Mock).mock.calls[0][0];
			...
			it('should apply role and estado filters', async () => {
				...
				await repository.findAll({ role: UserRole.ADMINISTRADOR, estado: UserState.ACTIVO });
		...
		describe('countAll', () => {
			it('should count users without filters', async () => {
				(mockPrismaClient.usuario.count as jest.Mock).mockResolvedValue(10);

				const result = await repository.countAll();

				expect(result).toBe(10);
				expect(mockPrismaClient.usuario.count).toHaveBeenCalledWith({ where: {} });
			});

			it('should count users with search filter', async () => {
				(mockPrismaClient.usuario.count as jest.Mock).mockResolvedValue(3);

				const result = await repository.countAll({ search: 'juan' });

				expect(result).toBe(3);
				const callArgs = (mockPrismaClient.usuario.count as jest.Mock).mock.calls[0][0];
				expect(callArgs.where).toEqual({
					OR: [
						{ nombres: { contains: 'juan', mode: 'insensitive' } },
						{ correo: { contains: 'juan', mode: 'insensitive' } },
