...
describe('Organizations module', () => {
	...
	describe('UpdateOrganizationUseCase', () => {
		...
		it('should update organization with new valid name', async () => {
			const updateDto: UpdateOrganizationDto = { nombre: 'Tech Corp Updated' };
		...
		it('should throw error when organization not found', async () => {
			...
			await expect(useCase.execute('org-nonexistent', {})).rejects.toThrow(
		...
		it('should validate new RUC with SUNAT', async () => {
			...
			expect(mockRepository.findByRuc).toHaveBeenCalledWith('20987654321');
			expect(mockSunatService.validateRuc).toHaveBeenCalledWith('20987654321');
		});
		...
		it('should reject if new RUC already exists', async () => {
			...
			(mockRepository.findById as jest.Mock).mockResolvedValue(orgForTest);
			(mockRepository.findByRuc as jest.Mock).mockResolvedValue(conflictingOrg);
		...
		it('should reject invalid RUC', async () => {
			...
			await expect(useCase.execute('org-1', updateDto)).rejects.toThrow(InvalidRucException);
			expect(mockRepository.save).not.toHaveBeenCalled();
		});
		...
		it('should update multiple fields', async () => {
			...
			(mockRepository.findById as jest.Mock).mockResolvedValue(existingOrg);
			const updatedOrg = { ...existingOrg, ...updateDto, updatedAt: new Date() };
		...
		it('should call rename method when nombre changes', async () => {
			const updateDto: UpdateOrganizationDto = { nombre: 'New Company Name' };
		...
		it('should call updateCommercialName when nombreComercial changes', async () => {
			const updateDto: UpdateOrganizationDto = { nombreComercial: 'New Commercial Name' };
			...
			const updateCommercialNameSpy = jest.spyOn(existingOrg, 'updateCommercialName');

			await useCase.execute('org-1', updateDto);

			expect(updateCommercialNameSpy).toHaveBeenCalledWith(updateDto.nombreComercial);
		});
