	...
	describe('PrismaOrganizationRepository', () => {
		...
		describe('save', () => {
			it('should create new organization when ID is empty', async () => {
				...
				(mockPrisma.organizacion!.create as jest.Mock).mockResolvedValue(mockOrgData);
			...
			it('should update existing organization when ID exists', async () => {
				...
				(mockPrisma.organizacion!.update as jest.Mock).mockResolvedValue(mockOrgData);
		...
		describe('findById', () => {
			it('should return organization when found', async () => {
				(mockPrisma.organizacion!.findUnique as jest.Mock).mockResolvedValue(mockOrgData);

				const result = await repository.findById('org-1');

				expect(mockPrisma.organizacion!.findUnique).toHaveBeenCalledWith({
					where: { id: 'org-1' },
				});
			...
			it('should return null when organization not found', async () => {
				(mockPrisma.organizacion!.findUnique as jest.Mock).mockResolvedValue(null);
		...
		describe('findByRuc', () => {
			it('should return organization when RUC found', async () => {
				(mockPrisma.organizacion!.findFirst as jest.Mock).mockResolvedValue(mockOrgData);
			...
			it('should return null when RUC not found', async () => {
				(mockPrisma.organizacion!.findFirst as jest.Mock).mockResolvedValue(null);
		...
		describe('findAll', () => {
			it('should return all organizations', async () => {
				const orgs = [mockOrgData, { ...mockOrgData, id: 'org-2', nombre: 'Company 2' }];
				(mockPrisma.organizacion!.findMany as jest.Mock).mockResolvedValue(orgs);
			...
			it('should return empty array when no organizations exist', async () => {
				(mockPrisma.organizacion!.findMany as jest.Mock).mockResolvedValue([]);
