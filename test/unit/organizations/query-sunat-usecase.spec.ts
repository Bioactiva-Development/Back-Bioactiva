...
describe('Organizations module', () => {
    describe('QuerySunatUseCase', () => {
        ...
        it('should query by RUC when input is 11 digits', async () => {
            ...
            expect(mockSunatService.getByRuc).toHaveBeenCalledWith(
                '20123456789',
            );
        ...
        it('should query by business name when input is not 11 digits', async () => {
            ...
            expect(mockSunatService.getByRazonSocial).toHaveBeenCalledWith(
                'Tech Corp',
            );
        ...
        it('should propagate service errors', async () => {
            mockSunatService.getByRazonSocial.mockRejectedValue(
                new Error('SUNAT unavailable'),
            );

            await expect(useCase.execute('Tech Corp')).rejects.toThrow(
                'SUNAT unavailable',
            );
        });
    });
