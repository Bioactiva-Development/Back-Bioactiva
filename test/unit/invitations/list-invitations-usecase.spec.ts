...
describe('Invitations module', () => {
	describe('ListInvitationsUseCase', () => {
		...
		it('should list invitations with all params', async () => {
			...
			const result = await useCase.execute(1, 10, 'test', TokenStatus.PENDIENTE);

			expect(result).toEqual(invitations);
			expect(mockRepository.list).toHaveBeenCalledWith(1, 10, 'test', TokenStatus.PENDIENTE);
		});
		...
		it('should list invitations without optional params', async () => {
			...
			expect(mockRepository.list).toHaveBeenCalledWith(undefined, undefined, undefined, undefined);
		});
