...
describe('Invitations module', () => {
	describe('ExpireOverdueInvitationsUseCase', () => {
		...
		it('should expire overdue invitations and return count', async () => {
			const inv1 = new InvitationToken(1, 'user1@test.com', 'token1', UserRole.TRABAJADOR, 1, TokenStatus.PENDIENTE, new Date(), null, new Date(Date.now() + 3600000));
			const inv2 = new InvitationToken(2, 'user2@test.com', 'token2', UserRole.TRABAJADOR, 1, TokenStatus.PENDIENTE, new Date(), null, new Date(Date.now() + 3600000));
		...
		it('should propagate repository errors', async () => {
			mockRepository.findPendingExpired.mockRejectedValue(new Error('DB error'));

			await expect(useCase.execute(new Date())).rejects.toThrow('DB error');
		});
	});
});
