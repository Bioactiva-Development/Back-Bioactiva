...
describe('Invitations module', () => {
	...
	describe('CreateInvitationUseCase', () => {
		...
		it('should create invitation for valid request', async () => {
			...
			expect(invitationPolicy.canCreateInvitation).toHaveBeenCalledWith(admin);
			expect(invitationPolicy.isAllowedDomain).toHaveBeenCalledWith(newEmail);
		...
		it('should reject if actor is not admin', async () => {
			...
			await expect(
				useCase.execute(worker, 'user@bioactiva.com', UserRole.TRABAJADOR),
		...
		it('should reject if pending invitation already exists', async () => {
			...
			await expect(
				useCase.execute(admin, 'user@bioactiva.com', UserRole.TRABAJADOR),
			).rejects.toThrow(InvalidInvitationDomainException);
		});
		...
		it('should schedule expiration and send notification', async () => {
			...
			expect(invitationExpirationScheduler.scheduleExpiration).toHaveBeenCalled();
			expect(invitationNotification.enqueueInvitationEmail).toHaveBeenCalled();
		});
	...
	describe('AcceptInvitationUseCase', () => {
		...
		it('should reject with invalid token', async () => {
			...
			await expect(
				useCase.execute('invalid-token', 'password123', 'Juan', 'Perez'),
		...
		it('should reject if invitation is expired', async () => {
			...
			invitationsRepository.findByToken.mockResolvedValue(expiredInvitation);
		...
		it('should hash password when creating user', async () => {
			...
			await useCase.execute('raw-token', 'plainpassword', 'Juan', 'Perez');
	...
	describe('ExpireInvitationUseCase', () => {
		...
		it('should return false if invitation not pending', async () => {
			...
			invitationsRepository.findById.mockResolvedValue(consumedInvitation);
